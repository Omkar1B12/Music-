...
import com.example.musicstream.models.CategoryModal
...
class MainActivity : AppCompatActivity() {
    ...
    fun showPlayerView(){
        ...
        MyExoplayer.getCurrentSong()?.let {
            ...
            Glide.with(binding.songCoverImageView).load(it.coverUrl).apply(
    ...
    fun getCategories(){
        FirebaseFirestore.getInstance().collection("category")
            .get().addOnSuccessListener{
                val categoryList=it.toObjects(CategoryModal::class.java)
                setupCategoryRecyclerView(categoryList)
            }
    }
    fun setupCategoryRecyclerView(categoryList: List<CategoryModal>){
    ...
    fun setupSection(id:String,mainLayout:RelativeLayout,titleView: TextView,recyclerView: RecyclerView){
        FirebaseFirestore.getInstance().collection("sections")
            .document(id)
            .get().addOnSuccessListener {
                val section=it.toObject(CategoryModal::class.java)
                section?.apply {
